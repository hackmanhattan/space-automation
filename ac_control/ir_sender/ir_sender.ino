...
#define PIR_PIN 3  // PIR sensor signal wired to the Trinket M0 pad labeled "3"
#define MOTION_TIMEOUT 1800000  // 30 minutes in milliseconds (30 * 60 * 1000)
...
#define STATUS_ARMED()   statusColor(0, 0, 0)     // off     - armed, no current motion
...
void setup() {
  ...
  pinMode (PIR_PIN, INPUT_PULLDOWN);  // pull low so a disconnected/idle line reads LOW (SAMD/ESP32)
#else
  pinMode (PIR_PIN, INPUT);           // AVR/ESP8266 have no internal pulldown
#endif
  Serial.begin(9600);
  IrSender.begin(1);
...
void loop() {
  ...
  // Reflect the *current* state on the status LED every loop, so it tracks the
  // sensor live instead of latching green until the timeout.
  if (ac_is_off) {
    STATUS_IDLE();          // dim red - timed out, AC switched off
  } else if (motion_detected == HIGH) {
    STATUS_MOTION();        // green   - motion present right now
  } else {
    STATUS_ARMED();         // off     - occupied window, no current motion
  }
